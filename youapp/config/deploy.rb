...
# the application.
set :application, "yywt"

# Target directory for the application on the web and app servers.
set :deploy_to, "/var/www/apps/#{application}"

# Primary domain name of your application. Used as a default for all server roles.
set :domain, "yywt.railsmachina.com"

# Login user for ssh.
set :user, "deploy"

# URL of your source repository.
set :repository, "svn+ssh://#{user}@#{domain}/home/deploy/svn/repos/#{application}/trunk"
...
desc "Simple head count of active yywt users"
task :user_count, :roles => :app do
  action = "rake user:fetch RAILS_ENV=production"
  run "cd %s/%s && %s" % [deploy_to,current_dir,action]
end
desc "Simple cache of counts for comm users"
task :cache_comm_count, :roles => :app do
  action = "rake cache:comm_user_count RAILS_ENV=production"
  run "cd %s/%s && %s" % [deploy_to,current_dir,action]
end
